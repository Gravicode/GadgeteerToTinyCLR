	...
	public class LedStrip : GTM.Module {
		...
		public bool this[int index] {
			get {
				...
				return this.leds[index].Read()==GpioPinValue.High;
			}
		...
		public LedStrip(int[] DigitalPin3To9) {
            ...
            this.leds = new GpioPin[this.LedCount]; //new GTI.DigitalOutput[this.LedCount];
			for (int i = 0; i < this.LedCount; i++) {
				this.leds[i] = controller.OpenPin(DigitalPin3To9[i]);//GTI.DigitalOutputFactory.Create(socket, Socket.Pin.Three + i, false, this);
                this.leds[i].SetDriveMode(GpioPinDriveMode.Output);
            }
        }
		...
		public void SetLed(int led, bool state) {
			...
			this.leds[led].Write(state?GpioPinValue.High:GpioPinValue.Low);
		}
